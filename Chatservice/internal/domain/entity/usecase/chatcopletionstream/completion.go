...
import (
	...
	"github.com/devfullcycle/fclx/chatservice/internal/domain/entity/gateway"
...
type ChatCompletionUseCase struct{
	ChatGateway gateway.ChatGateway // usar para chamar banco de dados
	OpenAiClient *openai.Client // usar para chamar a completion
...
func (uc *ChatCompletionUseCase) Execute(ctx context.Context, input ChatCompletionInputDTO) (*ChatCompletionOutputDTO, error) {
	...
	if err != nil {
		if err.Error() == "chat not founf"{
			// create new chat (entity)** Criação de novo chat da (entity)
			chat, err = createNewChat(input)
			if err != nil {
				return nil, errors.New("error creating new chat" + err.Error())
			}
		    // save on database** salvas banco de dados
	...
	userMessage, err := entity.NewMenssage("user", input.UserMassage, chat.Config.Model)
	if err != nil {
		return nil, errors.New("error creating new message: " + err.Error())
	}
	err = chat.AddMessege(userMessage)
	if err != nil {
	...
	assistant, err := entity.NewMenssage("assistant", fullResponse.String(), chat.Config.Model)
	if err != nil {
		return nil, errors.New("error creating assistant message: " + err.Error())
	}
	err = chat.AddMessege(assistant)
	if err != nil{
		return nil, errors.New("error adding new message: " + err.Error())
	}
...
func createNewChat(input ChatCompletionInputDTO) (*entity.Chat, error){
	...
	initialMessage, err := entity.NewMenssage("system", input.Config.InitialSystemMessage, model)
	if err != nil {
